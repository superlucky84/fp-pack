...
export const PipeStrict_ko = () => (
  <div class="prose prose-lg dark:prose-invert max-w-none">
    ...
    <p class="text-sm md:text-base text-gray-700 dark:text-gray-300 leading-relaxed mb-6">
      ...
      는 <strong>pipe</strong>의 엄격 버전입니다. 단계 간 타입 불일치를 더 빠르게 잡아내며,
      값-우선 <code class="text-sm">pipeStrict(data, ...)</code> 사용을 권장합니다. 재사용이 필요하면
      함수-우선 형태를 사용하세요.
    </p>
  ...
  (n) => String(n)
    ...
    <div class="bg-amber-50 dark:bg-amber-900/20 p-4 mb-6 rounded border border-amber-200 dark:border-amber-800 mt-6">
      <p class="text-sm md:text-base text-amber-900 dark:text-amber-200 leading-relaxed">
        <span class="font-medium">✅ pipeStrict는 언제 사용하나요?</span>
        <br />
        <br />
        순수 파이프라인에서 더 엄격한 타입 검사가 필요하면{' '}
        <code class="bg-amber-100 dark:bg-amber-900/40 px-1 py-0.5 rounded">pipeStrict</code>를 사용하세요.
        대부분의 경우에는 <code class="bg-amber-100 dark:bg-amber-900/40 px-1 py-0.5 rounded">pipe</code>로 충분합니다.
      </p>
    </div>
    ...
    <div class="border-l-4 border-yellow-500 bg-yellow-50 dark:bg-yellow-900/20 p-4 mb-6 rounded-r">
      <p class="text-sm md:text-base text-yellow-800 dark:text-yellow-200 leading-relaxed">
        ...
        <br />
        <br />
        현재 <strong>pipeStrict</strong> 문서를 보고 계십니다. 타입 추론의 유연성보다 타입 안정성이
        더 중요할 때, 특히 코드의 핵심 경로에서 이 함수를 사용하세요. 만약 타입 검사가 너무
        제약이 많다고 느껴진다면,{' '}
        <a
...
function pipeStrict<A, B>(
  ab: (a: A) => B
    ...
    <p class="text-sm md:text-base text-gray-700 dark:text-gray-300 leading-relaxed mb-6">
      <code class="text-sm">pipeStrict</code> 구현을 GitHub에서 확인하세요.
    </p>
    ...
    <div class="grid gap-6 mt-6">
      ...
      >
        ...
        <p class="text-sm md:text-base text-gray-700 dark:text-gray-300">
          엄격한 SideEffect 유니온을 제공하는 파이프라인.
        </p>
      </a>
