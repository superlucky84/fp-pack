...
const addMeta = assoc('meta', { active: true, tags: ['new'] } as Meta) as (user: UserBase) => UserWithMeta;
const addFlag = merge({ flagged: true }) as (user: UserWithMeta) => UserWithFlags;
const pickUser = pick(['id', 'meta', 'flagged'] as Array<'id' | 'meta' | 'flagged'>) as (user: UserWithFlags) => UserPick;
...
export type PipeObjectMetaIsStrict = Expect<Equal<typeof pipeObjectMeta, PipeObjectMetaExpected>>;
...
const setTheme = (value: ThemeState) => assocPath<ThemeState>(['profile', 'prefs', 'theme'], 'dark', value);
const clearTheme = dissocPath<ThemeState>(['profile', 'prefs', 'theme']);
const readTheme = (value: ThemeState) => path<string>(['profile', 'prefs', 'theme'], value);
...
export type PipeObjectThemeIsStrict = Expect<Equal<typeof pipeObjectTheme, PipeObjectThemeExpected>>;
...
const dropMeta = omit(['meta'] as Array<'meta'>) as (user: NamedUser) => Omit<NamedUser, 'meta'>;
...
export type PipeObjectHasNameIsStrict = Expect<Equal<typeof pipeObjectHasName, PipeObjectHasNameExpected>>;
...
const bumpCount = (value: Counter) => evolve({ count: (count: number) => count + 1 }, value);
...
export type PipeObjectEvolveIsStrict = Expect<Equal<typeof pipeObjectEvolve, PipeObjectEvolveExpected>>;
...
const addMetaMaybe = assoc('meta', { active: false, tags: [] } as Meta) as (user: MaybeUser) => MaybeUserWithMeta;
const hasNameMaybe = has('name') as (user: MaybeUser) => boolean;
const stripMeta = omit(['meta'] as Array<'meta'>) as (user: MaybeUserWithMeta) => MaybeUserWithoutMeta;
const getNameMaybe = prop('name') as (user: MaybeUserWithoutMeta) => string | undefined;
...
type PipeSideEffectObjectExpected = (input: MaybeUser | SideEffect<any>) => string | undefined | SideEffect<any>;
export type PipeSideEffectObjectIsStrict = Expect<Equal<typeof pipeSideEffectObject, PipeSideEffectObjectExpected>>;
...
const setRole = (value: Account) => assocPath<Account>(['profile', 'role'], 'member', value);
...
type PipeSideEffectStrictObjectResultExpected = Account | SideEffect<'NO_ID' | 'NO_ROLE'>;
export type PipeSideEffectStrictObjectResultIsStrict = Expect<
  Equal<typeof pipeSideEffectStrictObjectResult, PipeSideEffectStrictObjectResultExpected>
>;

type PipeSideEffectStrictObjectEffects = EffectUnion<typeof pipeSideEffectStrictObjectResult>;
type PipeSideEffectStrictObjectEffectsExpected = 'NO_ID' | 'NO_ROLE';
export type PipeSideEffectStrictObjectEffectsIsStrict = Expect<
  Equal<PipeSideEffectStrictObjectEffects, PipeSideEffectStrictObjectEffectsExpected>
>;

type PipeSideEffectStrictObjectValue = ValueUnion<typeof pipeSideEffectStrictObjectResult>;
...
const addDefaults = (value: AsyncConfig) => mergeDeep({ flags: { active: true } }, value);
const bumpAsyncCount = (value: AsyncConfig) => evolve({ count: (count: number) => count + 1 }, value);
...
const getAmount = prop('amount') as (value: Payment & { meta: Meta }) => number | undefined;
...
const setValue = (value: Payload) => assocPath<Payload>(['data', 'value'], 1, value);
...
type PipeAsyncSideEffectStrictObjectResultExpected = Promise<Payload | SideEffect<'NO_ID' | 'NO_VALUE'>>;
export type PipeAsyncSideEffectStrictObjectResultIsStrict = Expect<
  Equal<typeof pipeAsyncSideEffectStrictObjectResult, PipeAsyncSideEffectStrictObjectResultExpected>
>;

type PipeAsyncSideEffectStrictResolved = Awaited<typeof pipeAsyncSideEffectStrictObjectResult>;
type PipeAsyncSideEffectStrictEffects = EffectUnion<PipeAsyncSideEffectStrictResolved>;
type PipeAsyncSideEffectStrictEffectsExpected = 'NO_ID' | 'NO_VALUE';
export type PipeAsyncSideEffectStrictEffectsIsStrict = Expect<
  Equal<PipeAsyncSideEffectStrictEffects, PipeAsyncSideEffectStrictEffectsExpected>
>;

type PipeAsyncSideEffectStrictValue = ValueUnion<PipeAsyncSideEffectStrictResolved>;
export type PipeAsyncSideEffectStrictValueIsStrict = Expect<Equal<PipeAsyncSideEffectStrictValue, Payload>>;
